            ...
            <div class="content-wrapper">
                ...
                <div class="content-header">
                    <div class="container-fluid">
                        <div class="row mb-2">
                            <div class="col-sm-8">
                                <h1 class="m-0 text-dark"><b><u>WP PeopleSoftInfo Report</u></b></h1>
                            </div><!-- /.col -->
                            <div class="col-sm-4">
                                <?php 
                                $sqlGetUser = 'SELECT MAX(company_id) as maxid FROM company';
                                $resultGetUser = $conn->query($sqlGetUser);
                                $rowGetUser = $resultGetUser->fetch_assoc();

                                $sqlGetCompany = 'SELECT * FROM company WHERE company_id = '.$rowGetUser['maxid'];
                                $resultGetCompany = $conn->query($sqlGetCompany);
                                $rowGetCompany = $resultGetCompany->fetch_assoc();

                                ?>
                                <b>From:</b> <?php  echo $rowGetCompany['company_nm'].",";  ?><br>
                                <?php  echo $rowGetCompany['address'].",";  ?><br>
                                <?php  echo $rowGetCompany['phone'].",";  ?><br>
                            ...
                            <?php
                                $i=1;
                                $sqlUsers = 'SELECT DISTINCT Client FROM lsuform';
                                        ...
                                        <table class="table table-responsive">
                                            <thead class="thead-dark">
                                                <tr>
                                                  ...
                                                  <th scope="col">PeopleSoft Acct</th>
                                                  <th scope="col">PeopleSoft Fund</th>
                                                  <th scope="col">PeopleSoft Dept</th>
                                                  <th scope="col">PeopleSoft Program</th>
                                                  <th scope="col">PeopleSoft Class</th>
                                                  <th scope="col">PeopleSoft Project</th>
                                            ...
                                            <tbody>';
                                    $sql = 'SELECT * FROM lsuform WHERE Client = "'.$rowUsers['Client'].'"';
                                    $result = $conn->query($sql);
                                    while($row = $result->fetch_assoc()){
                                        if (isset($row['TestType']) && $row['TestType'] != '') {
                                            $sqlType = 'SELECT * FROM testtype WHERE type_id = '.$row['TestType'];
                                            $resultType = $conn->query($sqlType);
                                            $rowType = $resultType->fetch_assoc();
                                            $testtype = $rowType['type_nm'];
                                        }
                                        else{
                                            $testtype = '';
                                        }
