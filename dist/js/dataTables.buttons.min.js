...
;(function (e) {
  ...
  e.extend(w.prototype, {
        ...
        if (m)
          if (Array.isArray(m)) this._expandButton(a, m, c, d)
            ...
            l &&
              ...
              l.conf.buttons &&
                ((l.collection = e('<' + this.c.dom.collection.tag + '/>')),
                (l.conf._collection = l.collection),
                this._expandButton(l.buttons, l.conf.buttons, !0, d)),
    ...
    _buildButton: function (a, b) {
      ...
      var g = function (n, p, v, y) {
        ...
        e(p.table().node()).triggerHandler('buttons-action.dt', [
          p.button(v),
          p,
          v,
          y,
    ...
    _popover: function (a, b, c) {
      ...
      e('body')
        .on('click.dtb-collection', function (C) {
          ...
          ;((!e(C.target).parents()[I]().filter(a).length &&
            !e(J).hasClass('dt-buttons')) ||
            e(C.target).hasClass('dt-button-background')) &&
            k()
        })
      ...
      h = function (f) {
        if (Array.isArray(f)) for (var k = 0, g = f.length; k < g; k++) h(f[k])
          ...
          'string' === typeof f
            ? -1 !== f.indexOf(',')
              ? h(f.split(','))
              : ((f = e.inArray(f.trim(), d)), -1 !== f && c.push(b[f].inst))
        ...
        h = function (g, m) {
          ...
          if (Array.isArray(g) || g instanceof e)
            ...
            for (p = 0, l = n.length; p < l; p++)
              c.push({ inst: m, node: n[p].node })
          else if ('number' === typeof g)
            c.push({ inst: m, node: m.s.buttons[g].node })
          else if ('string' === typeof g)
            if (-1 !== g.indexOf(','))
              for (n = g.split(','), p = 0, l = n.length; p < l; p++)
                h(n[p].trim(), m)
            else if (g.match(/^\d+(\-\d+)*$/))
              ...
              for (g = g.replace(':name', ''), p = 0, l = n.length; p < l; p++)
                n[p].name === g && c.push({ inst: m, node: n[p].node })
            else
              e(p)
                .filter(g)
                .each(function () {
  ...
  t.Api.register('buttons()', function (a, b) {
    ...
    var c = this.iterator(
      ...
      function (d) {
        if (d._buttons)
          return w.buttonSelector(w.instanceSelector(a, d._buttons), b)
      },
      !0,
    )
    c._groupSelector = a
  ...
  t.Api.register('button().add()', function (a, b) {
    var c = this.context
    c.length &&
      ((c = w.instanceSelector(this._groupSelector, c[0]._buttons)),
  ...
  var G = function (a) {
    ...
    P = function (a, b) {
      ...
      k = k.nodes().toArray()
      for (
        var g = b.length, m = [], l = 0, n = 0, p = 0 < g ? f.length / g : 0;
        n < p;
        n++
      ) {
        for (var v = [g], y = 0; y < g; y++)
          (v[y] = c.format.body(f[l], n, y, k[l])), l++
        m[n] = v
      }
  ...
  e(z).on('init.dt plugin-init.dt', function (a, b) {
    'dt' === a.namespace &&
      (a = b.oInit.buttons || t.defaults.buttons) &&
      !b._buttons &&
      new w(b, a).container()
  })
